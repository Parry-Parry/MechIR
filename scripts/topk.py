...
def topk(
    ...
    output_all_file = (
        f"{out_path}/{formatted_model_mame}_{model_type}_{perturbation_type}_all.tsv"
    )
    if os.path.exists(output_all_file):
        ...
        queries = (
            pd.DataFrame(DL19_dataset.queries_iter())
            .set_index("query_id")
            .text.to_dict()
        )
        queries.update(
            pd.DataFrame(DL20_dataset.queries_iter())
            .set_index("query_id")
            .text.to_dict()
        )
        ...
        for rel_grade in range(max_rel + 1):
            ...
            original_scores = rel_data[~rel_data.perturbed].set_index(["qid", "docno"])[
                "score"
            ]
            perturbed_scores = rel_data[rel_data.perturbed].set_index(["qid", "docno"])[
                "score"
            ]
            ...
            score_deltas["text"] = score_deltas.apply(
                lambda x: text_lookup[(x.qid, x.docno, False)], axis=1
            )
            score_deltas["perturbed_text"] = score_deltas.apply(
                lambda x: text_lookup[(x.qid, x.docno, True)], axis=1
            )
            score_deltas["query"] = score_deltas.apply(
                lambda x: queries[str(x.qid)], axis=1
            )
