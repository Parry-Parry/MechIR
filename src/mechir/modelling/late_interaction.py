...
from .dot import get_hooked

logger = logging.getLogger(__name__)


def late_interaction(q: torch.Tensor, d: torch.Tensor):
    assert len(d.shape) == 3, "d should be of shape (batch, seq, model)"
    s = q @ d.permute(0, 2, 1)
    s = s.max(1).values
    s = s.sum(-1)
...
class LateInteraction(HookedRootModule, PatchedMixin, SAEMixin):
    def __init__(
        self,
    ...
    def score(self, queries: dict, documents: dict, reps_q=None, cache=False):
        ...
        if cache:
            ...
            return late_interaction(reps_q, reps_d), reps_q, reps_d, cache_d
        reps_d = self.forward(documents["input_ids"], documents["attention_mask"])
        return late_interaction(reps_q, reps_d), reps_q, reps_d
