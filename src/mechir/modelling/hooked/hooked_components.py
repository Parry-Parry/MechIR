...
class BertEmbed(nn.Module):
    ...
    def __init__(self, cfg: Union[Dict, HookedTransformerConfig]):
        ...
        self.token_type_embed = TokenTypeEmbed(self.cfg) if self.cfg.use_token_type_ids else nn.Identity()
        self.ln = LayerNorm(self.cfg)

        self.hook_embed = HookPoint()
        self.hook_pos_embed = HookPoint()
        self.hook_token_type_embed = HookPoint() if self.cfg.use_token_type_ids else nn.Identity()
    ...
    def forward(
        ...
        ) if self.cfg.use_token_type_ids else torch.zeros_like(word_embeddings_out)
